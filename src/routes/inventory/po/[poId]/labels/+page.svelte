<script lang="ts">
  ...
  $effect(() => {
    for (const b of poBatches) {
      if (qtyMap[b.id] === undefined) qtyMap[b.id] = 1;
    }
  });
      ...
      {#each poBatches as batch (batch.id)}
        ...
        <section class="rounded-lg border border-slate-200 bg-white p-4 print:hidden">
          <div class="mb-3 flex items-center justify-between gap-3">
            <div class="min-w-0">
              <div class="font-mono text-sm font-medium text-slate-900">{batch.code}</div>
            ...
            <div class="flex shrink-0 items-center gap-2">
              ...
              <input
                ...
                min="1"
                max="999"
                aria-label="Jumlah label untuk batch {batch.code}"
          ...
          {#if qty > 1}
            <p class="mt-2 text-xs text-slate-500">
              <span class="font-medium text-slate-700">{qty}×</span> label identik akan dicetak untuk batch ini.
            </p>
          {/if}
        </section>
      {/each}
