...
class ProductsStore {
  ...
  async load(): Promise<void> {
    ...
    try {
      ...
      // Backend returns raw Product shape with the JSON arrays/objects already
      // in the right shape — no transform needed.
      this.items = list as unknown as Product[];
      this.loaded = true;
    } finally {
      this.loading = false;
    }
  ...
  async update(
    ...
    // Backend update is full-payload — merge cached row with patch.
    const merged: ProductInput = { ...before, ...patch } as ProductInput;
